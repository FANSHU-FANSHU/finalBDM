...
if __name__=='__main__':
  ...
  rdd.mapPartitionsWithIndex(filfilter).reduceByKey(lambda x,y: (x[0]+y[0], x[1]+y[1]))\
  .mapValues(lambda x: x[0]/x[1]).map(lambda x: (x[0][0],(x[0][1],x[1]))).groupByKey().mapValues(outforma).sortBy(lambda x: x[0]).saveAsTextFile('Output3')
