...
def find_median(values):
  try:
    median = np.median(list(values))
    return round(float(median),14) 
  except Exception:
        return None
              ...
              if not (x[19].strip('{}') is ''):
                for each in x[19].strip('{}').split(','):
                  ...
                  if location_poi and location_home:
                    ...
                    for i in range(int(round(visitor))):
                      yield ((x[18],date),distance)
...
if __name__=='__main__':
    ...
    weeklypattern = sc.textFile("/tmp/bdm/weekly-patterns-nyc-2019-2020/*").mapPartitionsWithIndex(filfilter).groupByKey().mapValues(find_median)\
    .map(lambda x: (x[0][0],(x[0][1],x[1]))).groupByKey().mapValues(outforma).sortBy(lambda x: x[0])\
    .map(lambda x: (x[0],x[1][0],x[1][1],x[1][2],x[1][3]))

    header = sc.parallelize([('cbg_fips','2019-03','2019-10','2020-03','2020-10')])
    output1 = header.union(weeklypattern).map(toCSVLine).saveAsTextFile('ExtraResults.csv')